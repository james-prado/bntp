...
const BookmarkEditorComponent = ({ open, bookmark, shortcutKey, onRequestClose }) => {
    ...
    const closeAfter = (f) => void f()
        .then(onRequestClose)
        .catch((e) => setErrorMessage(String(e)));
    return (<DialogComponent className="BookmarkEditor" open={open} onRequestClose={onRequestClose}>
      <FormComponent editingBookmark={editingBookmark} editingShortcutKey={editingShortcutKey} setEditingBookmark={setEditingBookmark} setEditingShortcutKey={setEditingShortcutKey} errorMessage={errorMessage} onRequestClose={onRequestClose} onSubmit={() => closeAfter(async () => {
            await updateBookmark(editingBookmark);
            setShortcutMap(shortcutMap.set(editingBookmark.id, shortcutKey));
        })} onRemove={() => closeAfter(async () => {
...
const FormComponent = ({ editingBookmark, editingShortcutKey, setEditingBookmark, setEditingShortcutKey, errorMessage, onSubmit, onRemove, }) => {
    const favicon = useContext(FaviconContext);
    return (<form className="BookmarkEditor__Form" onSubmit={(e) => {
            onSubmit();
            e.preventDefault();
        }}>
      <input type="text" value={editingBookmark.title} required={true} autoFocus={true} onChange={(e) => setEditingBookmark({ ...editingBookmark, title: e.target.value })}/>
      <input type="text" value={editingBookmark.url} required={true} className="BookmarkEditor__Url" style={{ backgroundImage: `url(${favicon.getImageUrl(editingBookmark.url) ?? ''})` }} onChange={(e) => setEditingBookmark({ ...editingBookmark, url: e.target.value })}/>
      <input type="text" value={editingShortcutKey ?? ''} maxLength={1} placeholder="Shortcut Key (not assigned)" onChange={(e) => setEditingShortcutKey(shortcutKeyOf(e.target.value))}/>
