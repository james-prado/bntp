export const isBookmarkFolderIDArray = (value) => Array.isArray(value) && value.every((item) => typeof item === 'string');
export const filterBookmarks = (bookmarks, search) => {
    if (!search) {
        return bookmarks;
    }
    const searchLower = search.toLocaleLowerCase();
    return bookmarks.filter((b) => b.title.toLocaleLowerCase().includes(searchLower) || b.url.toLocaleLowerCase().includes(searchLower));
};
